...
### Trust anchor — 日本 + 台湾の threshold trustees

上の v2「分散 trustee」の行は単なる工学的抽象ではない。我々が目指す構成は、閾値復号ノードを **日本と台湾** に置き、独立した学術・非営利 trustee を加えた `k-of-n` スキーム。二つの民主主義、二つの独立した法域、市民インフラ整合性への利害一致、市民社会への合同監視の前歴なし。個別投票が漏れるためには両政府が自国民に対して協調行動を取る必要があり、政治的非現実性の閾値として実際に articulable なもの。

これは「運営者の約束としての信頼」ではなく、**地政学と暗号の合成としての信頼**。World ID credential 上で東アジアの市民投票を運ぶ primitive の鍵を誰が持つべきか、という問いへの最も誠実な答え: 我々でもなく、単一の財団でもなく、離反コストが国家レベル評判で測られる地政学的に分散したセット。
...
この primitive の最初の市場は米国でも EU でもなく、**台湾、そして日本**。

**台湾ファースト (2026 H2)**。3 つの条件が同時に揃っている稀な場所: World ID がすでに live（Orb、Credentials、政府 ID 統合が 2025 年以降デプロイ済み）、市民テックエコシステムが世界最成熟（g0v、vTaiwan、Pol.is、Join platform、Audrey Tang の Plurality フレームワーク）、V-Dem 指数上で民主主義国中最大の外国情報戦負荷 — これが投票秘匿を "nice to have" ではなく構造的インフラにしている。2026 年 11 月の地方選挙が自然なパイロット窓口。vohu は Plurality の投票レイヤ。

**日本セカンド (2027)**。マイナンバーカード × World ID 統合（2025 年 11 月以降 live）が 1.2 億人以上の国民に対して主権 ID と検証済み人間 credential を結びつけている。日本の政策ステークホルダーから World ID ベースインフラへの前向きな反応が確認されている（Tokyo cowork メンターより）。台湾の先行事例が通常日本政府との対話を遅らせる先行採用リスクを解消する。国家規模展開への経路は通常より短い。

**韓国、EU、米国は後続** — 東アジアの実証が確立し、閾値 trustee アーキテクチャが日本・台湾ノードに分散されてから（上の *Trust anchor* 節参照）。

これは推測ではない: 経路が要求する技術要素はすべて production に存在している。残る仕事は政治的シーケンシングと成功裏のパイロット — それが実際の次の 5 年の building work で、マーケティング資料ではない。
